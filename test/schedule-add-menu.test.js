...
describe('Phase 1B — "+ Lägg till" primary menu', () => {
  ...
  it('hides iOS native type=time chrome and shows Starttid/Sluttid until a time is chosen', () => {
    ...
    const render = src.slice(src.indexOf('function renderTimeField'), src.indexOf('function restoreSearchFocus'));
    assert.match(render, /schedule\.startTimePlaceholder/);
    assert.match(render, /schedule\.endTimePlaceholder/);
    assert.match(render, /sam-time-field/);
    assert.match(render, /sam-time-value/);
    assert.match(render, /opacity-0/);
    assert.match(render, /id="samActivityStartTime"/);
    assert.match(render, /id="samActivityEndTime"/);
    assert.doesNotMatch(render, /new Date\(|toTimeString|getHours/);
