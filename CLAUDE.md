...
- 2026-05-29: Bugfix — Apple Sign In 401 (SW v146): `_jwkToPem()` in auth.js replaced with `crypto.createPublicKey({ key: jwk, format: 'jwk' })` — manual DER encoding was producing invalid PEM; login.html/register.html 409 handlers fixed (`data.code` → `data.error`); SW cache bumped to v146. Files: src/routes/auth.js, public/login.html, public/register.html, public/sw.js, CLAUDE.md.
