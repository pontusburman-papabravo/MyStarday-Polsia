...
- 2026-05-28: Hotfix — Child dashboard crash (v130 regression): batch ratings query wrapped in try/catch + LEFT JOIN via unnest so GET `/api/me/daily-log` never 500s; removed loadDay generation guard that could skip render; background reload errors no longer wipe visible schedule. SW v131. Files: src/routes/daily-logs.js, public/js/child-dashboard.js, public/sw.js, CLAUDE.md.
- 2026-05-28: Bugfix — Child check-off + mood rating: removed dev-only `kanslo_tracking` gate from rating modal (controlled by `show_mood_rating` per child); fixed wrong `emotion_tracking` slug in init; serialized `toggleItem` + coalesced `loadDay` to prevent race errors when checking multiple tasks; batch ratings in GET `/api/me/daily-log`. SW v130. Files: public/js/child-dashboard.js, public/child-dashboard.html, src/routes/daily-logs.js, public/sw.js, CLAUDE.md.
...
- 2026-05-28: Feature — APNs push backend: sendAPNs() implemented with raw HTTP/2 + ES256 JWT auth (no new npm deps); BadDeviceToken/Unregistered tokens auto-deleted from push_subscriptions; POST /api/admin/test-push with family_id targeting; docs at docs/app-store-apns.md. Files: src/lib/push-notifications.js, db/push-subscriptions.js, src/routes/admin/system.js, docs/app-store-apns.md, CLAUDE.md.
- 2026-05-28: Feature — Native push-notifier iOS: push-manager.js detekterar `Platform.isNative()` i init() och detourerar subscribe()/unsubscribe()/isSubscribed() till Platform.push på native iOS/Android; auto-register på app-start; logout rensar push-token före session; settings.html döljer PWA-guiden på native. SW v119. Files: public/js/push-manager.js, public/js/auth.js, public/settings.html, public/sw.js, CLAUDE.md.
- 2026-05-27: Feature — Fas 4 go-live: pedagoganteckningar feature-flag satt till 'live' (alla familjer); SW v112→v113 för PWA cache refresh; penetrationstest (20/20 kod-nivå checks gröna: requireNotPedagogOnly på family/children/rewards routes, revoked_at IS NULL i parent-access/pedagog-notes/pedagog-invite, is_draft=false i getOverview, ghost-draft upsert, SW v113 deployed live). test-penetration.js verifieringsskript. Pontuss familj (beta) redo för manuell funktionsacceptance: invite pedagog → e-post → accept → /pedagog-oversikt. Files: public/sw.js, test-penetration.js, CLAUDE.md.
